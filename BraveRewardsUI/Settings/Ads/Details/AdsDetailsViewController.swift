...
class AdsDetailsViewController: UIViewController {
  ...
  func generateRows() {
    ...
    if state.ads.shouldAllowAdsSubdivisionTargeting {
...
extension AdsDetailsViewController: UITableViewDelegate, UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if row == .adsPerHour {
      ...
      guard let selectedIndex = subdivisionTargetingOptions.firstIndex(where: { $0.0 == state.ads.adsSubdivisionTargetingCode }) else { return }

      let controller = OptionsSelectionViewController(
        options: subdivisionTargetingOptions.map { $0.value },
        selectedOptionIndex: selectedIndex) { [weak self] (selectedIndex) in
          guard let self = self else { return }
          self.state.ads.adsSubdivisionTargetingCode = self.subdivisionTargetingOptions[selectedIndex].0
          self.navigationController?.popViewController(animated: true)
      }
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    case .subdivisionTargeting:
      ...
      if state.ads.adsSubdivisionTargetingCode == "AUTO" {
        adsSubdivisionTargetingCode = state.ads.automaticallyDetectedAdsSubdivisionTargetingCode
      } else {
        adsSubdivisionTargetingCode = state.ads.adsSubdivisionTargetingCode
      }
